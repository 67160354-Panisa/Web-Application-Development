...
function ClosetPage({setToast}) {
  ...
  const load=()=>api.closet().then(d=>{
    if(!Array.isArray(d)) { setItems([]); } else { setItems(d); }
  ...
  return <section className="page">
    ...
    {show && <ClosetModal close={()=>setShow(false)} save={async(formData)=>{
      try{
        ...
        setShow(false);
        setToast("เพิ่มเสื้อผ้าเข้าตู้แล้ว ✨");
        load();
      }catch(e){setToast(e.message)}
    }}/>}
  </section>
...
function ClosetModal({close,save}) {
  const [form,setForm]=useState({name:"",category:"top",color:"black",size:"M",style:"casual",condition:"good"});
  ...
  return <Modal title="Add to your closet" close={close}>
    <form className="form" onSubmit={submit}>
      <Field label="Item name" value={form.name} onChange={v=>setForm({...form,name:v})} placeholder="e.g. Baby tee" required/>
      <div className="two-col">
        <Select label="Category" value={form.category} onChange={v=>setForm({...form,category:v})} options={categories.map(x=>[x.key,x.label])}/>
        <Select label="Color" value={form.color} onChange={v=>setForm({...form,color:v})} options={["black","white","cream","pink","blue","brown","grey","green"].map(x=>[x,x])}/>
      </div>
      <div className="two-col">
        <Select label="Style" value={form.style} onChange={v=>setForm({...form,style:v})} options={["casual","korean","y2k","minimal","street","preppy"].map(x=>[x,x])}/>
        <Select label="Size" value={form.size} onChange={v=>setForm({...form,size:v})} options={["XS","S","M","L","XL"].map(x=>[x,x])}/>
      </div>
      <label className="field">
        <span>Upload Image</span>
        <input type="file" accept="image/*" onChange={e => setFile(e.target.files[0])} style={{padding: '8px 0'}} />
      </label>
      <button className="btn btn-dark full">Save piece <Check size={17}/></button>
    </form>
  </Modal>
}
...
function MarketPage({user,setToast}) {
  ...
  const load=()=>api.products(`?q=${encodeURIComponent(q)}${type?`&listing_type=${type}`:""}`).then(d=>{
    if(!Array.isArray(d)){ setProducts([]); } else { setProducts(d); }
  }).catch(e=>setToast(e.message));
  
  useEffect(()=>{ load(); api.closet().then(d=>setCloset(Array.isArray(d)?d:[])).catch(()=>{}); },[q,type]);
  
  const buy=async(p)=>{if(String(p.id).startsWith("demo")){setToast("ทดสอบการซื้อจริงให้ลงขายจาก My Closet ก่อนนะ");return}try{await api.createOrder({product_id:p.id,quantity:1});setToast("สั่งซื้อสำเร็จ 🎉");load()}catch(e){setToast(e.message)}};
  
  // ฟังก์ชันลบสินค้า
  const del=async(id)=>{
    if(!confirm("ต้องการยกเลิกการขายสินค้านี้ใช่ไหม?"))return;
    try{
      const token=localStorage.getItem("closetloop_token");
      const res=await fetch(`http://localhost:8000/products/${id}`,{method:"DELETE",headers:{"Authorization":`Bearer ${token}`}});
      if(!res.ok) throw new Error("ลบไม่สำเร็จ");
      setToast("ยกเลิกการขายเรียบร้อย 🗑️");
      load();
    }catch(e){setToast(e.message)}
  };
  ...
  return <section className="page">
    ...
    <div className="product-grid market-products">{products.length?products.map(p=><ProductCard key={p.id} product={p} onBuy={()=>buy(p)} onDelete={()=>del(p.id)} currentUser={user}/>):demoProducts.map(p=><ProductCard key={p.id} product={p} demo onBuy={()=>buy(p)}/>)}</div>
...
function ProductCard({product,onBuy,onDelete,demo,currentUser}) {
  ...
  return <article className="product-card">
    <div className="product-art">
      {product.image_url?<img src={product.image_url}/>:emoji}
      <span className="product-label">{product.listing_type==="exchange"?"EXCHANGE":"FOR SALE"}</span>
      {/* ถ้าเป็นสินค้าของเรา เปลี่ยนปุ่มหัวใจเป็นปุ่มลบ */}
      {isOwner ? (
        <button className="heart" onClick={(e)=>{e.stopPropagation(); onDelete();}} style={{color:'var(--ink)'}}><Trash2 size={16}/></button>
      ) : (
        <button className="heart"><Heart size={16}/></button>
      )}
    </div>
    <div className="product-info"><div><b>{product.title}</b><small>{product.description||"Pre-loved piece from the community"}</small></div><strong>{money(product.price)}</strong></div>
    {onBuy&&<button className="buy-btn" onClick={onBuy} disabled={isOwner}>{isOwner?"Your listing":"Buy now"} <ArrowRight size={15}/></button>}
  </article>
}
