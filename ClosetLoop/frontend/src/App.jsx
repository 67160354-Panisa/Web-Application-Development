...
function App() {
  ...
  // เช็คว่ามี Token รีเซ็ตรหัสผ่านติดมากับ URL ไหม
  ...
  // ถ้าเข้าลิงก์มาจากอีเมล ให้เปิดหน้าตั้งรหัสใหม่
  ...
  const submit = async (e) => {
    ...
    try {
      if (mode === "forgot-password") {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.detail || "เกิดข้อผิดพลาด");
        alert(data.message); // แจ้งเตือนให้ไปดูใน Terminal
        setMode("login");
      } else if (mode === "login") {
      ...
      <div className="auth-panel">
        <div className="auth-panel-inner">
          ...
          <form onSubmit={submit} className="form">
            ...
            {/* โหมดลืมรหัสผ่านโชว์แค่ช่องอีเมล */}
...
function Dashboard({setPage,user}) {
  ...
  useEffect(()=>{api.closet().then(setCloset).catch(()=>{}); api.products().then(setProducts).catch(()=>{});},[]);
...
function ClosetPage({setToast}) {
  ...
  const [show,setShow]=useState(false);
  const load=()=>api.closet().then(setItems).catch(e=>setToast(e.message));
  useEffect(load,[]);
...
function AIPage({setToast}) {
  ...
  const [busy,setBusy]=useState(false);
  useEffect(()=>{api.closet().then(setCloset).catch(()=>{})},[]);
  const generate=async()=>{if(!closet.length){setToast("เพิ่มเสื้อผ้าใน My Closet ก่อนนะ");return}setBusy(true);try{setResult(await api.generateOutfit(form));}catch(e){setToast(e.message)}finally{setBusy(false)}};
  return <section className="page">
...
function MarketPage({user,setToast}) {
  ...
  const load=()=>api.products(`?q=${encodeURIComponent(q)}${type?`&listing_type=${type}`:""}`).then(setProducts).catch(e=>setToast(e.message));
  useEffect(()=>{load();api.closet().then(setCloset).catch(()=>{})},[q,type]);
